...
<div class="search-content">
	<?if (count($arResult["SEARCH_DOCUM"]) > 0 && strlen($_REQUEST['q']) > 0):?>
		...
		<div class="search-results">
			<?foreach ($arResult["SEARCH_DOCUM"] as $arItem):?>
			<div class="search-result-item clearfix">
				<div class="title">
					<a href="<?echo $arItem["FILE"]?>" target="_blank"><?echo $arItem["NAME"]?></a>
				</div>
				<?if (count($arItem["DIRECTION"]) > 0 && is_array($arItem["DIRECTION"])):?>
				<div class="result-category">
					<?echo implode(" | ", $arItem["DIRECTION"]);?>
				</div>
				<?endif;?>
			</div>
			<?endforeach;?>
	...
	<?elseif(strlen($_REQUEST['q']) <= 0):?>
		<div class="search-result-item clearfix">
			<div class="title">
				Воспользуйтесь формой для поиска нужного Вам документа.
			</div>
		</div>	
	<?else:?>
		<div class="search-result-item clearfix">
...
<?
#DebugMessage($arResult["SEARCH_DOCUM"]);
?>