					...
					default://CHECKBOXES
						...
						<?foreach($arItem["VALUES"] as $val => $ar):?>
							<div class="field custom_checkbox">
			                    <input type="checkbox" id="checkbox_<?=$ar["CONTROL_ID"]?>" value="<? echo $ar["HTML_VALUE"] ?>" name="<? echo $ar["CONTROL_NAME"] ?>" <? echo $ar["CHECKED"]? 'checked="checked"': '' ?> onclick="smartFilter.click(this)"/>
			                    <label for="checkbox_<?=$ar["CONTROL_ID"]?>"><? echo $ar["VALUE"]; ?></label>
			                </div>
