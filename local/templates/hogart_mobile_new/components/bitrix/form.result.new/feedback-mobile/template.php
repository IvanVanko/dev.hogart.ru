...
<?=$arResult["FORM_HEADER"]?>
	<?foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion):?>
		...
		<div class="field">
			...
			<?=$arQuestion["HTML_CODE"]?>
			<?if ($arResult["isFormErrors"] == "Y"):?>
				<?#DebugMessage($arQuestion['STRUCTURE'][0]["FIELD_TYPE"]);?>
				<?if (is_array($arResult["FORM_ERRORS"]) && array_key_exists($FIELD_SID, $arResult['FORM_ERRORS'])):?>
				<script>
					$('input[name="form_<?=$arQuestion['STRUCTURE'][0]["FIELD_TYPE"]?>_<?=$arQuestion['STRUCTURE'][0]["ID"]?>"]').addClass("error");
					//$('select[name="form_<?=$arQuestion['STRUCTURE'][0]["FIELD_TYPE"]?>_<?=$arQuestion['STRUCTURE'][0]["ID"]?>"]').addClass("error");
				</script>
				<?else:?>
				<script>
					$('input[name="form_<?=$arQuestion['STRUCTURE'][0]["FIELD_TYPE"]?>_<?=$arQuestion['STRUCTURE'][0]["ID"]?>"]').addClass("success");
					//$('select[name="form_<?=$arQuestion['STRUCTURE'][0]["FIELD_TYPE"]?>_<?=$arQuestion['STRUCTURE'][0]["ID"]?>"]').addClass("success");
				</script>
				<?endif;?>
			<?endif;?>
		</div>
...
#DebugMessage($arResult["FORM_ERRORS"]);
?>
<script>
$(document).ready(function () {
	//$('form[name="<?=$arResult["arForm"]["SID"]?>"]').addClass("equpment-form");
	$('form[name="<?=$arResult["arForm"]["SID"]?>"]').addClass("message-form");
	$('form[name="<?=$arResult["arForm"]["SID"]?>"]').addClass("green-form");
	$('input[name="form_text_4"]').inputmask("+7 (999)999-99-99");
