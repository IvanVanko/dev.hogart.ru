...
<?=$arResult["FORM_HEADER"]?>
	<?foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion):?>
		...
		?>
		<div class="field <?=$cls ?>">
			<label for="add-file" class="input-btn">Приложить резюме<?#=$arQuestion["CAPTION"]?></label>
			<input type="file" id="add-file" name="form_file_<?=$arQuestion['STRUCTURE'][0]['ID']?>" accept="application/pdf,application/msword,text/plain" class="add-file hide">
...
$(document).ready(function () {
	$('form[name="<?=$arResult["arForm"]["SID"]?>"]').addClass("equpment-form");
	 ...
	 $('input[name="form_text_9"]').inputmask("+7 (999)999-99-99");
