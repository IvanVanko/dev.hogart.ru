...
?>
    <?foreach ($arResult["QUESTIONS"] as $code => $arQuestion) {
        ...
        if ($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden')
        {
            ...
            <label for="<?=$arQuestion["STRUCTURE"]['HTML_ID']?>" class="<?=($type == 'file') ? "add_file":""?>"><?=!empty($arParams['CUSTOM_CAPTION'][$code]) ? $arParams['CUSTOM_CAPTION'][$code] : $arQuestion['CAPTION']?><?if ($arQuestion['REQUIRED'] == 'Y') {?>*<?}?></label>
