...
$arParams['NAME_TEMPLATE'] = empty($arParams['NAME_TEMPLATE']) ? (method_exists('CSite', 'GetNameFormat') ? CSite::GetNameFormat() : "#NAME# #LAST_NAME#") : $arParams["NAME_TEMPLATE"];
    ...
    if ($_REQUEST["delete"] && check_bitrix_sessid()) {
        ...
        if (is_array($ARR_RESULT) && count($ARR_RESULT) > 0 && check_bitrix_sessid()) {
            ...
            foreach ($ARR_RESULT as $del_id) {
                ...
                if ($del_id > 0)
                    CFormResult::Delete($del_id); // rights check inside
            }
