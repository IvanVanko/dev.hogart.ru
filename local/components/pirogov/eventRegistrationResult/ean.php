...
class EAN13 {
    ...
    public function __construct($number, $scale) {
        $this->PARITY_KEY   = unserialize(PARITY_KEY);

        $this->LEFT_PARITY  = unserialize(LEFT_PARITY);
        $this->RIGHT_PARITY = unserialize(RIGHT_PARITY);
        $this->GUARD        = unserialize(GUARDS);
    ...
    protected function _encode() {
        ...
        for($i = 1; $i <= strlen($this->number) - 1; $i++) {
            if($i < 7) {
                $barcode[] = $this->LEFT_PARITY[$this->_key[$i - 1]][substr($this->number, $i, 1)];
            }
            else {
                $barcode[] = $this->RIGHT_PARITY[substr($this->number, $i, 1)];
            }
            if($i == 6) {
