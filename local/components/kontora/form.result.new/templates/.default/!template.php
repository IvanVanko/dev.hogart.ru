...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized())
{
	...
	else // if there;s no any errors - create user
	{
		...
		if (intval($ID) > 0)
		{
			...
			$rsUsers = CUser::GetList(($by="ID"), ($order="desc"), array("ID"=>$ID),array("SELECT"=>array("UF_*")));
			if($arUser = $rsUsers->Fetch()){
				foreach($arUser["UF_PHOTO_PROMO"] as $key => $PhotoId){
					$arEventFields['PHOTO_PROMO'][] = "http://".$_SERVER["HTTP_HOST"].CFile::GetPath($PhotoId);
				}
			}

			$event = new CEvent;
			$event->SendImmediate("NEW_USER", SITE_ID, $arEventFields);
			$event->SendImmediate("NEW_TEST_PARTICIPANTS", SITE_ID, $arEventFields);
			$event->SendImmediate("TEST_PARTICIPANTS_INFO", SITE_ID, $arEventFields);
			if($bConfirmReq)
