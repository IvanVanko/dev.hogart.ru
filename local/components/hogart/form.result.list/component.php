...
if (CModule::IncludeModule("form"))
{
    ...
    if ($arParams["arFormInfo"] = $dbres->Fetch())
    {
        ...
        {
            // Находим форму по ID что узнать SEMINAR_REGISTRATION_NUMBER
            $arAnswer = CFormResult::GetDataByID($arR['ID'], array(), $arResult, $arAnswer2);
            $user_text = reset($arAnswer2['SEMINAR_REGISTRATION_NUMBER'])['USER_TEXT'];
            $arResult['FILE_SEND'] = reset($arAnswer2['FILE_SEND'])['USER_TEXT'];

            // По ранее найденному SEMINAR_REGISTRATION_NUMBER получаем всех пользователей
            $arFields[] = array("CODE" => "SEMINAR_REGISTRATION_NUMBER", "VALUE" => $user_text, "EXACT_MATCH" => "Y");
            $arFilte["FIELDS"] = $arFields;
            $rsResults = CFormResult::GetList($arParams["WEB_FORM_ID"], $arParams["by"], $arParams["order"], $arFilte, $arResult["is_filtered"]);
